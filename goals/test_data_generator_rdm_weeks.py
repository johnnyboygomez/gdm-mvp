#!/usr/bin/env python3
"""
Complex Test Data Generator - Variable Week 2 Length

Enhanced version that randomly generates 3-7 days for week 2:
- Week 1: Always 7 days → generates goal
- Week 2: Randomly 3-7 days → only generates goal if 7 days, otherwise empty {}
"""
...
def create_week_data(start_monday, step_values, num_days=7):
    """Convert step values to JSON format for specific week (variable length)"""
    week_data = []
...
def create_test_pair_complex(name, initial_week_start, week1_base, week2_base=None, variation=200):
    """
    Create a complex test pair with variable week 2 length:
    1. Single week test (always 7 days)
    2. Same data shifted back + variable length week 2 (3-7 days)
    """
    
    # Generate week 1 step values (always 7 days)
    week1_values = generate_week_values(week1_base, variation)
    week1_avg = calculate_average(week1_values)
    
    # TEST 1: Single week (always 7 days)
    week1_data = create_week_data(initial_week_start, week1_values, 7)
    ...
    # TEST 2: Same data shifted back + variable length week 2
    # Shift week 1 back by 7 days
    shifted_week_start = initial_week_start - timedelta(days=7)
    shifted_week1_data = create_week_data(shifted_week_start, week1_values, 7)  # SAME VALUES!
    
    # Generate week 2 data with random length (3-7 days)
    week2_days = random.randint(3, 7)
    ...
    week2_data = create_week_data(initial_week_start, week2_values, week2_days)
    ...
    # Second goal: only if week 2 has full 7 days, otherwise empty
    if week2_days == 7:
        week2_avg = calculate_average(week2_values[:7])  # Use all 7 days
        expected_goal_2 = predict_goal(week2_avg, "second_week")
    else:
        expected_goal_2 = {}  # Empty goal for incomplete weeks
    ...
    return {
        ...
        'week2_days': week2_days,  # Track how many days week 2 had
        'week2_complete': week2_days == 7  # Whether week 2 was complete
    }

def generate_random_test_datasets(num_datasets=20, seed=42):
    """Generate multiple random test datasets with varied scenarios and variable week 2 length"""
    ...
    for i in range(num_datasets):
        ...
        all_test_results[test_name] = create_test_pair_complex(
            name=test_name,
...
def save_to_csv(all_test_results, filename="step_goals_test_data_complex.csv"):
    """Save test results to CSV file with week 2 complexity info"""
    ...
    for test_name, results in all_test_results.items():
        ...
        csv_data.append({
            ...
            'goal_2_increase': '',
            'goal_2_target': '',
            'week2_days': 7,  # Always 7 for single week tests
            'week2_complete': True,
            'validation_notes': 'Single week baseline test'
        })
        
        # Test 2 (Two weeks with variable length)
        test2_data = results['test2_data']
        test2_expected = results['test2_expected']
        week2_days = results['week2_days']
        week2_complete = results['week2_complete']
        ...
        week2_status = "COMPLETE" if week2_complete else f"INCOMPLETE ({week2_days} days)"
        validation_notes = f"First goal identical: {validation_status}, Week 2: {week2_status}"
        
        # Handle empty goal for incomplete week 2
        if week2_complete and goal_info_2_t2:
            goal_2_avg = goal_info_2_t2.get('average_steps', '')
            goal_2_increase = goal_info_2_t2.get('increase', '')
            goal_2_target = goal_info_2_t2.get('new_target', '')
            num_goals = 2
        else:
            goal_2_avg = ''
            goal_2_increase = ''
            goal_2_target = ''
            num_goals = 1  # Only first goal when week 2 incomplete
        
        csv_data.append({
            'test_pair': test_name,
            'test_type': 'two_weeks_variable',
            ...
            'goal_2_average_steps': goal_2_avg,
            'goal_2_increase': goal_2_increase,
            'goal_2_target': goal_2_target,
            'week2_days': week2_days,
            'week2_complete': week2_complete,
    ...
    fieldnames = [
        ...
        'week2_days', 'week2_complete', 'validation_notes'
    ]
...
def main():
    """Generate complex test datasets with variable week 2 length"""
    
    print("Complex Step Goals Test Data Generator")
    print("=====================================")
    print("Features:")
    print("• Week 1: Always 7 days → generates goal")
    print("• Week 2: Randomly 3-7 days → goal only if complete (7 days)")
    print("• Incomplete weeks get empty goal: {}")
    print()
    ...
    # Generate filename
    output_filename = f"step_goals_test_data_complex_{num_datasets}datasets.csv"
    
    print(f"\nGenerating {num_datasets} complex datasets with seed {seed}...")
    ...
    # Count complete vs incomplete week 2s
    complete_count = sum(1 for r in all_test_results.values() if r['week2_complete'])
    incomplete_count = num_datasets - complete_count
    ...
    print(f"Week 2 breakdown: {complete_count} complete (7 days), {incomplete_count} incomplete (3-6 days)")
